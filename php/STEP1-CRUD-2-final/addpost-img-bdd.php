...
if (isset($_FILES['image']) && $_FILES['image']['error'] == 0) {
    ...
    if ($typeMime === "image/jpeg") {
        ...
        if (move_uploaded_file($_FILES['image']['tmp_name'], $dossier . $nomFichier)) {
            ...
            // ------------------------------------------------------------
            // AUTRE SOLUTION : on enregistre le nom complet de l'image en BDD
            // ------------------------------------------------------------
            // Plutôt que de recalculer "id.jpg" dans le template à chaque affichage,
            // on stocke ici le nom réel du fichier dans la colonne "image".
            // Le template n'aura alors qu'à faire : echo $article['image'];
            $updateImage = $mysqlClient->prepare('
                UPDATE s2_articles_presse SET image = :image WHERE id = :id
            ');
            $updateImage->execute([
                'image' => $nomFichier,
                'id' => $article_id,
    ...
    <div class="container">
        ...
        <div class="card">
            <div class="card-body">
                ...
                <!-- On réutilise le nom de fichier tel qu'il a été stocké en BDD (colonne "image") -->
                <img src="img/<?php echo $nomFichier ?? $article_id . '.jpg'; ?>" alt="<?php echo $titre; ?>" class="img-fluid">
            </div>
        </div>
