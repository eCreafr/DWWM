...
Le test de l'étape 2 s'adapte à votre avancement : lancez-le dès le premier
`TODO` complété. Il vous dit ce qui passe, ce qui manque, et s'arrête à la
barrière « Niveau 1 validé » tant que `Dvd` et `JeuVideo` n'existent pas.
Relancez-le après chaque `TODO` plutôt qu'une seule fois à la fin.
...
## Ce que vous devez savoir expliquer à l'oral (restitution 16h30)
...
3. Comparez avec un fichier fourni qui fait quelque chose de similaire
   (votre `Livre.php` complété au niveau 1 de l'étape 2 est le modèle pour
   `Dvd.php` et `JeuVideo.php`).
